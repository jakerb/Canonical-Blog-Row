(function() {
	...
	/*
	 * @note `wp_api_url` is used to pull post data via WordPress API.
	 */
	var wp_api_url = 'https://admin.insights.ubuntu.com/wp-json/wp/v2/posts?per_page=3&page=1&_embed=True';

	/*
	 * @note Creating a view application and attaching it to `#blog`.
	 */
	new Vue({
	    el: '#blog',
	    created() {
	    ...
	    methods: {
	    	/* 
	    	 * @note Using Axios to request WP API data.
	    	 */
	        query_api() {
		        axios.get(wp_api_url).then(response => {

		        	/*
		        	 * @note Map each post item to return a structure post object.
		        	 */
